...
class Recommender:
    def __init__(self, k, batch_size=64):
        ...
        self.loss = 0.0
        self.prev_loss = 0.0
        self.item_similarity = self.compute_item_similarity()  # Added: Precompute item similarity matrix

    def compute_item_similarity(self):
        """
        Compute cosine similarity matrix between items
        Returns: n_items x n_items similarity matrix
        """
        ...
        for i in range(n_items):
            for j in range(i + 1, n_items):
                ...
                sim = max(0, sim)  # Ensure similarity is in the range [0, 1]
                similarity[i, j] = sim
                similarity[j, i] = sim
        return similarity
    ...
    def train_model(self, sock):
        ...
        while iteration < self.config.maxIter:
            ...
            ils_test = self.compute_ils(pred_ratings, n=10)  # Added ILS calculation
            diversity_test = 1 - ils_test  # Added Diversity calculation
