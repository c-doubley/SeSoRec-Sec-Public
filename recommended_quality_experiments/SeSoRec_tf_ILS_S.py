"""
File Name: SeSoRec_tf_ILS_S.py

Description:
    Reproduction version of SeSoRec, including the SSMM protocol
    Uses the TrustFilm dataset
    Corresponds to python SeSoRec_tf_ILS_R.py
    Adds ILS evaluation for Diversity
Functionality:
...
def ssmm_b(Q, sock):
    """
    Secure Matrix Multiplication Implementation for Platform B
    Q: Matrix held by Platform B
    sock: Socket for communication with Platform A
    """
    # Ensure Q is a numpy array and of floating-point type
    Q = np.array(Q, dtype=np.float64)
    
    Q = Q.T  # Transpose Q to match P * Q^T
    y, z = Q.shape
    pad_row = y % 2  # 1 if y is odd, 0 if even
    if pad_row:
        Q = np.pad(Q, ((0, 1), (0, 0)), 'constant')  # If y is odd, add a row of zeros
        y += 1
    
    # Step 1: Generate random matrix Q'
    Q_prime = np.random.rand(y, z)
    
    # Step 3: Extract even and odd rows of Q'
    Q_e = Q_prime[::2, :]  # Even rows
    Q_o = Q_prime[1::2, :]  # Odd rows
    
    # Step 5: Compute Q1 and Q2
    Q1 = Q_prime - Q
    Q2 = Q_e - Q_o
    ...
    # Ensure P1 and P2 are numpy arrays and of floating-point type
    P1 = np.array(P1, dtype=np.float64)
    P2 = np.array(P2, dtype=np.float64)
    
    # Send Q1 and Q2 to Platform A
    send_with_length(sock, (Q1, Q2))
    
    # Step 7: Compute N
    N = P1 @ (2 * Q - Q_prime) - P2 @ (Q2 + Q_e)
    
    # If a padding row was added earlier, remove it now
    if pad_row:
        N = N[:, :-1]
    
    # Step 8: Send N to Platform A
...
def client():
    ...
    for k in range(5):
        ...
        with client_socket:
            try:
                # Actively send TRAIN request to start the training process
                ...
                while True:
                    ...
                    if command == "GET_SOCIAL_USERS":
                        ...
                        # Should return user ratings here, but in Social.py we do not have rating data
        ...
        # Wait for the server to prepare for the next fold
        time.sleep(5)  # Increase wait time to ensure the server has enough time to restart
